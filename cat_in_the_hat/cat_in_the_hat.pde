//Credit To Will We worked together

float kittyEyesOffset = 0;
float kittyEyesDx = 0.2;
float xTower;
float w = 100;
...
void draw() {
  background(155);
  for (xTower = 1; xTower <=5; xTower += 1) {
    tower(xTower * 300, 300, 200, "kitty", color(222, 135, 12));
  }
  for (xTower = 1; xTower <=5; xTower += 1) {
    tower(xTower * 300 - 150, 300, 400, "Person", color(222, 135, 12));
  }
}
